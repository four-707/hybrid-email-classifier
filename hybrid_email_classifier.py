...
class gmail():
    def __init__(self):
        self.emails_list=[{}]
    def send_for_predict(self):
        emails = []
        for item in self.emails_list:
            s = ""
    ...
    def log_in(self):
        try:
            ...
            self.get_inbox()
            self.send_for_predict()
        except:
            messagebox.showerror("login unsuccesssful","Please enter valid credentials and check your internet connection")
...
class page2_mainpage:
    ...
    def label_frame1_content(self):
        ...
        b1.place(x=10, y=600)
        event_in_out(b1, bl)
        b_link_email=Button(self.lf1,text="Gmail-login", bg=bl,command=self.login_account)
        b_link_email.place(x=60,y=600)
        event_in_out(b_link_email, bl)
    ...
    def login_account(self):
        ...
        def submit():
            ...
            password_var.set("")
            self.tempf.destroy()
            # lis_emails = login_obj.log_in();
            Thread(target=login_obj.log_in).start()
            # Thread(target=self.call_predict, args=(email_lis2,)).start()

        font_siz=20
        self.tempf=Frame(self.lf2)
        self.tempf.place(x=50,y=50)
        cpop = Canvas(self.tempf, width=600,height=300, borderwidth=0, highlightthickness=0)
        cpop.pack()
        # cpop.pack(side=LEFT, fill=BOTH, expand="yes")
        float_frame=Frame(cpop,width=300,height=300)
        float_frame.place(x=50,y=50);

        name_label = Label(float_frame, text='Username', font=('calibre', font_siz, 'bold'))
        # creating a entry for input
        # name using widget Entry
        name_entry = Entry(float_frame, textvariable=email_var, font=('calibre', font_siz, 'normal'))
        # creating a label for password
        passw_label = Label(float_frame, text='Password', font=('calibre', font_siz, 'bold'))
        # creating a entry for password
        passw_entry = Entry(float_frame, textvariable=password_var, font=('calibre', font_siz, 'normal'), show='*')
        # creating a button using the widget
        # Button that will call the submit function
        sub_btn = Button(float_frame, text='Submit', font=('calibre', font_siz, 'normal'), command=submit)
